...
let handler = async (m, { conn, usedPrefix: _p }) => {
  try {
   ...
   conn.sendButton(m.chat, `*${wish()}, ${name} 👋*`, text.trim(), await genProfile(conn, m), [['Speedtest', _p + 'ping'], ['Owner', _p + 'owner']], false, { quoted: fkon, contextInfo: { externalAdReply: { showAdAttribution: true,
    mediaUrl: "https://Instagram.com/ryannn.store",
    mediaType: "VIDEO",
    description: "https://Instagram.com/ryannn.store", 
    title: '𝚈𝚊𝚗𝚗𝙼𝚍',
    body: wm,
    thumbnail: thumb,
    sourceUrl: sgc
...
async function genProfile(conn, m) {
  let font = await jimp.loadFont('./names.fnt'),
    ...
    avatar = await jimp.read(await conn.profilePictureUrl(m.sender, 'image').catch(() => 'https://telegra.ph/file/07e82d4a7bec7db6b3f3f.jpg')),
